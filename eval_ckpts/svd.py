...
'''
Evaluating the R50 network with low rank approximated final layer. 
Code is capable of running across different datasets. 
'''
...
ff =  2048
for K in [2**(i+3) for i in range(8)]:  
	print(f"Using Rank {K} approximation") # note that the rank <= 1000 
	args.feat_dim=ff
	model = resnet50(False)
	model.fc=FixedFeatureLayer(args.feat_dim, 1000)
	apply_blurpool(model)		
	model.load_state_dict(get_ckpt(args.path+f"sh=0_mh=0_nesting_start=3_fixed_feature=2048/final_weights.pt"))

	# Doing a low rank approximation 
	w = model.fc.weight.data
	U, S, V = torch.svd(w)
	W_hat = torch.matmul(torch.matmul(U[:, :K], torch.diag(S[:K])), (V.t())[:K, :])
	model.fc.weight.data = W_hat
